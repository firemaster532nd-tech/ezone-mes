...
export default defineConfig({
  ...
  plugins: [
    react(),
    VitePWA({
      registerType: 'autoUpdate',          // 배포 시 앱 자동 업데이트
      includeAssets: ['favicon.ico', 'apple-touch-icon.png', 'icons/*.png'],
      manifest: {
        name: 'EZONE MES — 이지원',
        short_name: '이지원MES',
        description: '(주)이지원 제조실행시스템 (MES) — 생산·품질·물류·회계 통합 관리',
        theme_color: '#0f172a',
      ...
      workbox: {
        ...
        runtimeCaching: [
          ...
          },
          {
            urlPattern: /\/api\/.*/i,
            handler: 'NetworkFirst',
            options: { cacheName: 'api-cache', expiration: { maxEntries: 200, maxAgeSeconds: 60 * 60 } },
          },
        ],
      },
      devOptions: {
