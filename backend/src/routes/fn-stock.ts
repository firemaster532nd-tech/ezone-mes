...
async function migrateFnStock() {
  await pool.query(`
    ...
    CREATE TABLE IF NOT EXISTS fn_finished_stock (
      ...
      diameter_mm INTEGER NOT NULL,       -- 100 / 75 / 50
      spec        VARCHAR(30) NOT NULL,   -- 몸통 / 150H / 170H / 180H / 190H / 200H / 210H / 240H / 250H / 260H
      qty         INTEGER DEFAULT 0,
      updated_at  TIMESTAMPTZ DEFAULT NOW()
    );
    CREATE UNIQUE INDEX IF NOT EXISTS uq_fn_finished ON fn_finished_stock(diameter_mm, spec);

    -- 부자재 재고 (보호철판, 볼트너트, 시트 등)
    CREATE TABLE IF NOT EXISTS fn_material_stock (
      stock_id    SERIAL PRIMARY KEY,
      item_name   VARCHAR(50) NOT NULL,   -- 보호철판 / 볼트,너트,와샤 / 시트(재단) / 시트(압출)
      spec        VARCHAR(30) NOT NULL,   -- 100파이 / 75파이 / 50파이 / -
      qty         INTEGER DEFAULT 0,
    ...
    CREATE TABLE IF NOT EXISTS fn_stock_tx (
      ...
      tx_type     VARCHAR(10) NOT NULL,   -- IN / OUT / ADJUST
...
export async function fnStockRoutes(app: FastifyInstance) {
  ...
  app.get('/api/fn-stock/finished', { preHandler: requireAuth }, async () => {
    const r = await pool.query(
      `SELECT * FROM fn_finished_stock ORDER BY diameter_mm DESC, spec`
    );
    return { data: r.rows };
  });
  ...
  app.get('/api/fn-stock/material', { preHandler: requireAuth }, async () => {
    const r = await pool.query(
      `SELECT * FROM fn_material_stock ORDER BY item_name, spec`
    );
    return { data: r.rows };
  });
  ...
  app.post('/api/fn-stock/receive', { preHandler: requireAuth }, async (req, reply) => {
    const { stock_type, diameter_mm, spec, item_name, qty, memo, created_by } = req.body as any;
    ...
    try {
      ...
      await client.query(
        ...
        [stock_type, stockId, itemLabel, spec || '', qty, memo || null, created_by || null]
      );

      await client.query('COMMIT');
      return { data: { success: true, message: `${itemLabel} ${qty}개 입고 완료` } };
  ...
  app.post('/api/fn-stock/ship', { preHandler: requireAuth }, async (req, reply) => {
    const { diameter_mm, spec, qty, memo, created_by } = req.body as any;
    ...
    try {
      ...
      await client.query(
        `INSERT INTO fn_stock_tx (tx_type, stock_type, stock_id, item_name, spec, qty, memo, created_by)
         VALUES ('OUT', 'FINISHED', $1, $2, $3, $4, $5, $6)`,
        [r.rows[0].stock_id, label, spec, qty, memo || null, created_by || null]
      );
      await client.query('COMMIT');
