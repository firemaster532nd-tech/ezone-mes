...
function calcBrackets(code: string, w: number, h: number, q: number) {
  ...
  switch (code) {
    ...
    case 'VT-01':
      ...
      break;
    case 'VAG-1.69':
      add(0.6, 60,  sw - 5, q * 4, '상하평철1 브라켓');
      add(0.6, 204, sw - 5, q * 4, '상하평철2 브라켓');
      add(0.6, 235, sw,     q * 2, '중앙받침대1');
      add(0.6, 190, sw,     q * 2, '중앙받침대2');
      break;
    case 'HTG-064': case 'HTG-064DC':
      add(0.6, 60,  w - 5,  q * 2, '상하평철1 브라켓');
      add(0.6, 274, w - 5,  q * 2, '상하평철2 브라켓');
      add(1.6, 60,  h - 35, q * 4, '좌우 브라켓');
      add(1.6, 50,  h,      q * 3, '보강대(소켓하부)');
      break;
    case 'HTG-1.69':
      add(0.6, 60,  sw,     q * 4, '상하평철1 브라켓');
      add(0.6, 274, sw,     q * 4, '상하평철2 브라켓');
      add(0.6, 318, sw + 5, q * 8, '중앙받침대1');
      add(0.6, 255, sw + 5, q * 8, '중앙받침대2');
      add(1.6, 60,  h - 35, q * 4, '좌우 브라켓');
