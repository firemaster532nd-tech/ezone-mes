...
const createUserSchema = z.object({
  ...
  // ë¹?ë¬¸ì???„ì†¡ ??undefined ë¡?ì²˜ë¦¬ ??Zod .email() ê²€ì¦??¤ë¥˜ ë°©ì?
  email: z.preprocess(
...
export function validatePassword(password: string): string | null {
  ...
  if (!/\d/.test(password)) {
    return 'ë¹„ë?ë²ˆí˜¸?ëŠ” ?«ìê°€ ë°˜ë“œ???¬í•¨?˜ì–´???©ë‹ˆ??';
  }
  if (!/[\W_]/.test(password)) {
    return 'ë¹„ë?ë²ˆí˜¸?ëŠ” ?¹ìˆ˜ë¬¸ìê°€ ë°˜ë“œ???¬í•¨?˜ì–´???©ë‹ˆ??';
  }
  if (/(\d)\1\1/.test(password)) {
    return 'ë¹„ë?ë²ˆí˜¸??3ê°??´ìƒ??ë°˜ë³µ???«ì(?? 111)ë¥??¬ìš©?????†ìŠµ?ˆë‹¤.';
  }
  for (let i = 0; i < password.length - 2; i++) {
    ...
    if (c1 >= 48 && c1 <= 57 && c2 >= 48 && c2 <= 57 && c3 >= 48 && c3 <= 57) {
      if ((c2 === c1 + 1 && c3 === c2 + 1) || (c2 === c1 - 1 && c3 === c2 - 1)) {
        return 'ë¹„ë?ë²ˆí˜¸??3ê°??´ìƒ???°ì†???«ì(?? 123, 321)ë¥??¬ìš©?????†ìŠµ?ˆë‹¤.';
      }
    }
  }
  return null;
...
export async function ensureAdminUser() {
  ...
  if (res.rows.length === 0) {
    ...
    await pool.query(
      ...
      ['?œìŠ¤??ê´€ë¦¬ì', 'admin', hash, deptId, 'admin', '?œìŠ¤??ê´€ë¦¬ì', true, true]
    );
...
export async function authRoutes(app: FastifyInstance) {
  // ?œë²„ ?œì‘ ??admin ê³„ì • ë³´ì¥ ë°??„ì§ ?¨ìŠ¤?Œë“œê°€ ?¸íŒ…?˜ì? ?Šì? ë¹„ê?ë¦¬ì ê³„ì •?¤ì˜ ì´ˆê¸° ë¹„ë?ë²ˆí˜¸ë¥??´ë???ë²ˆí˜¸ë¡??¸íŒ…
  ...
  app.post('/api/auth/login', async (req, reply) => {
    ...
    // ?€?€ ?ˆí¼ê´€ë¦¬ì ì²˜ë¦¬ (DB ì¡°íšŒ/ë¡œê·¸/IP ê¸°ë¡ ?„í? ?†ìŒ) ?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€?€
    ...
    // ë¹„ë?ë²ˆí˜¸ê°€ ?¼ì¹˜?˜ì? ?Šê³ , ?…ë ¥??ë¹„ë?ë²ˆí˜¸ê°€ ?˜ì´?ˆì´ ë¹ ì§„ ?´ë???ë²ˆí˜¸ ?•ì‹(10~11?ë¦¬ ?«ì)??ê²½ìš° ?˜ì´?ˆì„ ?£ì–´??ì¶”ê? ê²€ì¦??œë„
  ...
  // GET /api/auth/me  (?„ì¬ ë¡œê·¸?¸í•œ ?¬ìš©??+ ê¶Œí•œ ëª©ë¡)
  ...
  app.post('/api/auth/change-password', { preHandler: requireAuth }, async (req, reply) => {
    ...
    // ?„ì¬ ?„ì‹œ ë¹„ë?ë²ˆí˜¸ê°€ ?¼ì¹˜?˜ì? ?Šê³ , ?…ë ¥??ê°’ì¸ ?˜ì´?ˆì´ ë¹ ì§„ ?´ë???ë²ˆí˜¸ ?•ì‹(10~11?ë¦¬ ?«ì)??ê²½ìš° ?˜ì´?ˆì„ ?£ì–´??ì¶”ê? ê²€ì¦??œë„
  ...
  // GET /api/auth/users  (manager ?´ìƒ: ?„ì²´ ì§ì› ëª©ë¡ ì¡°íšŒ ??ê´€ë¦¬ì ?„ìš©)
  ...
  app.post('/api/auth/users', { preHandler: requireRole('admin') }, async (req, reply) => {
    ...
    // ë¹„ë?ë²ˆí˜¸ ê²€ì¦? ?´ë??°ë²ˆ??ê¸°ë°˜ ?„ì‹œ ë¹„ë?ë²ˆí˜¸??ê²½ìš°??ë³µì¡??ê¸°ì? ë©´ì œ
  ...
  app.delete<{ Params: { id: string } }>(
    ...
    async (req, reply) => {
      ...
      if (target.employee_no === 'admin') {
        return reply.code(403).send({ error: 'cannot_delete_admin', message: '?œìŠ¤??ê´€ë¦¬ì ê³„ì •?€ ?? œ?????†ìŠµ?ˆë‹¤.' });
      }
